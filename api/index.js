...
function mapArticle(row) {
  const tags = Array.isArray(row.tags) ? row.tags.map((t) => typeof t === "string" ? t : t.name).filter(Boolean) : row.tags || [];
  return {
    id: row.id,
    slug: row.slug || "",
    title: row.title,
...
async function getAllArticles(params = {}) {
  ...
  const articles = data.map((row) => {
    const tags = row.article_tags?.map((at) => at.tags?.name).filter(Boolean) || [];
...
async function getArticleById(id) {
  ...
  const tags = data.article_tags?.map((at) => at.tags?.name).filter(Boolean) || [];
  ...
  if (article.tags) {
    ...
    if (article.tags.length > 0) {
      for (const tagName of article.tags) {
        ...
        let { data: tagData, error: tagError } = await supabase.from("tags").select("id").eq("slug", tagSlug).single();
        let tagId;
        if (!tagData) {
...
async function saveMedia(media) {
  ...
  const { error } = await supabase.from("media").upsert(mediaData);
  if (error) {
...
async function saveGallery(gallery) {
  ...
  const { error } = await supabase.from("galleries").upsert(galleryData);
  if (error) {
    console.error("Error saving gallery:", error);
    throw error;
  }
}
async function deleteGalleryRow(id) {
...
async function getVideoGallery() {
  const { data, error } = await supabase.from("video_galleries").select("*").limit(1).single();
  if (error) {
...
async function saveVideoGallery(gallery) {
  const { error } = await supabase.from("video_galleries").upsert({
...
// server/lib/audit.ts
async function logAction(action, module, targetId, userName = "Vinay Admin") {
  try {
...
var getArticles = async (req, res) => {
  try {
    ...
    const limitQuery = req.query.limit ? parseInt(req.query.limit) : void 0;
    const offsetQuery = req.query.offset ? parseInt(req.query.offset) : 0;
...
var updateArticle = async (req, res) => {
  ...
  const updatedArticle = { ...existing, ...updateData };
...
var getMedia = async (req, res) => {
  const media = await getAllMedia();
  res.json({ media });
};
var uploadMedia = async (req, res) => {
  const file = req.file;
  if (!file) {
  ...
  try {
    const optimizedBuffer = await sharp(file.buffer).resize({ width: 2e3, withoutEnlargement: true }).webp({ quality: 85, effort: 6 }).toBuffer();
    const fileName = `${Date.now()}-${file.originalname.replace(/[^a-zA-Z0-9]/g, "_")}.webp`;
    const { data: uploadData, error: uploadError } = await supabase.storage.from(BUCKET_NAME).upload(fileName, optimizedBuffer, {
...
var getAnalytics = async (req, res) => {
  ...
  const monthlyViews = [
    { name: "Jan", views: 4e3, engagement: 2400 },
    { name: "Feb", views: 3e3, engagement: 1398 },
    { name: "Mar", views: 2e3, engagement: 9800 },
  ...
  const topArticles = articles.filter((a) => a.status === "Published").slice(0, 5).map((a) => ({
    id: a.id,
    title: a.title,
...
// server/routes/ai.ts
var generateContent = async (req, res) => {
  ...
  if (type === "complete") {
    ...
    res.json({
      ...
      keywords: [.../* @__PURE__ */ new Set([...keywords, "trending", "exclusive", "entertainment"])]
    });
  } else {
...
var getAds = async (req, res) => {
  const { data, error } = await db.from("ads").select("*").order("id");
  if (error) {
...
var updateAdStatus = async (req, res) => {
  const { id } = req.params;
...
var getMonetizationStats = async (req, res) => {
  const { data, error } = await db.from("ads").select("revenue_today, revenue_month");
  if (error) {
...
var getAuditLogs = async (req, res) => {
  const { data, error } = await db.from("audit_logs").select("*").order("timestamp", { ascending: false }).limit(100);
  if (error) {
...
var createCategory = async (req, res) => {
  ...
  const { data, error } = await db.from("categories").insert({ name, slug, description }).select().single();
  if (error) {
...
var updateCategory = async (req, res) => {
  ...
  const { name, description } = req.body;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-");
  const { data, error } = await db.from("categories").update({ name, slug, description }).eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
  ...
  await logAction("Updated Category", "Taxonomy", id);
  res.json(data);
};
var deleteCategory = async (req, res) => {
  const { id } = req.params;
  const { data, error } = await db.from("categories").delete().eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
      res.status(404).json({ message: "Category not found" });
      return;
  ...
  await logAction("Deleted Category", "Taxonomy", id);
  res.status(204).send();
};
...
var updateTag = async (req, res) => {
  ...
  const { name } = req.body;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-");
  const { data, error } = await db.from("tags").update({ name, slug }).eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
  ...
  await logAction("Updated Tag", "Taxonomy", id);
  res.json(data);
};
var deleteTag = async (req, res) => {
  ...
  await logAction("Deleted Tag", "Taxonomy", id);
  res.status(204).send();
};

// server/routes/comments.ts
var getComments = async (req, res) => {
  const { data, error } = await db.from("comments").select("*, articles(title)").order("created_at", { ascending: false });
  if (error) {
...
var updateCommentStatus = async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  const { data, error } = await db.from("comments").update({ status }).eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
  ...
  await logAction(`Updated Comment Status: ${status}`, "Moderation", id);
  res.json(data);
};
var deleteComment = async (req, res) => {
  const { id } = req.params;
  const { data, error } = await db.from("comments").delete().eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
      res.status(404).json({ message: "Comment not found" });
      return;
  ...
  await logAction("Deleted Comment", "Moderation", id);
  res.status(204).send();
};
...
var createUser = async (req, res) => {
  ...
  const { data, error } = await db.from("users").insert({ name, email, role }).select().single();
  if (error) {
    if (error.code === "23505") {
...
var updateUser = async (req, res) => {
  ...
  const { data, error } = await db.from("users").update({ name, email, role, status }).eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
  ...
  await logAction("Updated User Profile", "Identity", id);
  res.json(data);
};
var deleteUser = async (req, res) => {
  ...
  await logAction("Deleted User Account", "Identity", id);
  res.status(204).send();
};
...
var markAsRead = async (req, res) => {
  const { id } = req.params;
  const { data, error } = await db.from("notifications").update({ is_read: true }).eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
...
var deleteNotification = async (req, res) => {
  const { id } = req.params;
  const { data, error } = await db.from("notifications").delete().eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
      res.status(404).json({ message: "Notification not found" });
      return;
...
var createNotification = async (req, res) => {
  ...
  const { data, error } = await db.from("notifications").insert({ type, title, message, link }).select().single();
  if (error) {
...
var createIntegration = async (req, res) => {
...
var updateIntegration = async (req, res) => {
  ...
  await logAction(`Updated Integration: ${name}`, "System", id);
  res.json(data);
};
var deleteIntegration = async (req, res) => {
  const { id } = req.params;
  const { data, error } = await db.from("integrations").delete().eq("id", id).select().single();
  if (error) {
    if (error.code === "PGRST116") {
      res.status(404).json({ message: "Integration not found" });
      return;
  ...
  await logAction(`Purged Integration: ${data.name}`, "System", id);
  res.status(204).send();
};

// server/routes/galleries.ts
var getGalleries = async (req, res) => {
  try {
...
var createGallery = async (req, res) => {
  try {
...
var updateGallery = async (req, res) => {
  try {
    const { id } = req.params;
    const gallery = { ...req.body, id };
    await saveGallery(gallery);
    res.json({ success: true, gallery });
  } catch (error) {
...
var deleteGallery = async (req, res) => {
  try {
    const success = await deleteGalleryRow(req.params.id);
    res.json({ success });
  } catch (error) {
...
// server/routes/video-galleries.ts
...
var updateVideoGallery = async (req, res) => {
  try {
    const gallery = req.body;
...
function createServer() {
  try {
    ...
    app.get("/api/articles/:id", getArticleById2);
